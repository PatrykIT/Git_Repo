...
class Siatka:
    ...
    def pokaz_komorki(self, siatka, plansza_widoczna_dla_gracza, numer_rzedu, numer_kolumny):
        # Wyjdz z funkcji jesli komorka jest juz pokazana
        if plansza_widoczna_dla_gracza[numer_rzedu][numer_kolumny] != ' ':
            return

        # Pokaz obecna komorke
        plansza_widoczna_dla_gracza[numer_rzedu][numer_kolumny] = siatka[numer_rzedu][numer_kolumny]

        # Pobierz sasiadow jesli komorka jest pusta
        if siatka[numer_rzedu][numer_kolumny] == '0':
            for r, c in self.pobierz_sasiednie_komorki(numer_rzedu, numer_kolumny):
                # Powtorz funkcje dla kazdego sasiada ktory nie ma flagi
                if plansza_widoczna_dla_gracza[r][c] != 'F':
                    self.pokaz_komorki(siatka, plansza_widoczna_dla_gracza, r, c)
        ...
        while True:
            ...
            if komorka:
                ...
                # Jesli jest tu flaga, pokaz wiadomosc
                    ...
                    siatka.pokaz_komorki(siatka.plansza, siatka.plansza_widoczna_dla_gracza, numer_rzedu, numer_kolumny)
